...
def calcular_kg(pontos, elementos):
    ...
    for i in elementos:
        xi, yi, j, k = i.calcular_kge(pontos)
...
def resolver_sistema(A, B):
    ...
    assert np.shape(A)[0] == np.shape(B)[0], "As dimensoes das matrizes nao combinam!"

    try:
        resposta = np.linalg.solve(A, B)
    except np.linalg.LinAlgError:
        resposta, residuo, rank, s = np.linalg.lstsq(A, B) # Verifica-se se o sistema pode ser mal condicionado
...
#    pdb.set_trace()
    assert np.allclose(np.dot(A, resposta), B), "Erro! Valores calculados nao estao corretos!"
...
def expandir_xgr(Xr, pontos, rest):
    ...
    tipo_elem = Xr.dtype.type
    # Inicia-se X como um vetor de zeros
    X = np.zeros((ngl*len(pontos), 1), dtype=tipo_elem)
